...
chdir($builddir);
my $buildcommand = qq{java -classpath ../shrinksafe/js.jar:../shrinksafe/shrinksafe.jar org.mozilla.javascript.tools.shell.Main build.js profile=agua_all action=release cssOptimize=comments releaseDir=../../../builds/$build-agua_all version="0.6" > $installdir/html/builds/$build-agua_all.build.txt
};
