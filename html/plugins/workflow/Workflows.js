...
//	RETRIEVE MIN/MAX NODES FOR THE CLUSTER
	////console.log("Workflows.setClusterNodes    plugins.admin.Clusters.setClusterNodes(clusterName)");
	...
	var clusters = Agua.getClusters();
	////console.log("Workflows.setClusterNodes    clusters: " + dojo.toJson(clusters));
	clusters = this.filterByKeyValues(clusters, ["cluster"], [clusterName]);
	if ( clusters == null )	return;
	////console.log("Workflows.setClusterNodes    FILTERED clusters: " + dojo.toJson(clusters));
...
// SET MIN NODES VALUE TO SENSIBLE NUMBER 
	////console.log("Workflows.checkNodeNumbers     plugins.admin.Stages.checkNodeNumbers()");
...
//	SAVE A PARAMETER TO Agua.parameters AND TO REMOTE DATABASE
	////console.log("Workflows.saveClusterWorkflow    plugins.admin.Clusters.saveClusterWorkflow(event)");
	console.log("Workflows.saveClusterWorkflow    event: " + event);
	console.log("Workflows.saveClusterWorkflow    this.ready: " + this.ready);
	console.log("Workflows.saveClusterWorkflow    caller: " + this.saveClusterWorkflow.caller.nom);

	if ( ! this.ready ) {
		console.log("Workflows.saveClusterWorkflow    this.ready is FALSE. Returning");
		return;
	}
	//// AVOID SELF-TRIGGERING OF onChange EVENT WHEN CHANGED PROGRAMMATICALLY
	if ( this.savingCluster == true )	return;
