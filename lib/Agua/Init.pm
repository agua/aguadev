...
=cut
	...
	### PRINT KEYS TO FILE
	print "Agua::Init::init    Printing key files\n";
	$self->printKeyFiles();
...
#### MOUNT nethome CONTAINING USER HOME DIRECTORIES
	...
	$self->ops()->formatVolume($device, $filetype) if not $isvolume and not $issnapshot;
...
method restoreAdminHome ($mountpoint) {
	...
	$self->logError("Can't move adminhome: $tempadminhome to mountpoint: $mountpoint") and exit if -d $tempadminhome;
