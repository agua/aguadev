...
# Booleans
has 'warn'		=> ( isa => 'Bool', is => 'rw', required	=>	0	);
has 'help'		=> ( is  => 'rw', 'isa' => 'Bool', required	=>	0, documentation => "Print help message"	);
has 'backup'	=> ( is  => 'rw', 'isa' => 'Bool', default	=>	0, documentation => "Automatically back up files before altering"	);
...
#### RUN COMMAND LOCALLY OR ON REMOTE HOST
	...
	if ( defined $self->ssh() ) {
		...
		$self->logDebug("error", $error) if defined $error and $error;
		
		#### CHECK FOR PROCESS ERRORS
		$self->logError("Error with command: $command ... $@") and exit if $@ ne"" and not $self->warn();
