...
method cloneRemoteRepo ($owner, $repository, $hubtype, $login, $keyfile, $target) {
	$self->logDebug("keyfile", $keyfile);
	my $prefix = $self->getPrefix($login, $hubtype, $keyfile);
	my $command = "$prefix git clone git\@github.com:$owner/$repository.git $target  2>&1 ";
	$self->logDebug("command", $command);
...
#### DO FETCH THEN HARD RESET (E.G., TO AVOID pull CONFLICTS)
	...
	my $command = "$prefix git fetch git\@github.com:$owner/$repository.git ";
	$self->logDebug("command", $command);

	my ($output, $error) = $self->runCommand($command);

	#### GET TAGS
	$command = "$prefix git fetch git\@github.com:$owner/$repository.git --tags ";
	$self->logDebug("command", $command);
...
method pullRemoteRepo ($owner, $repository, $hubtype, $login, $privacy, $keyfile) {
	...
	if ( $privacy eq "private" ) {
		$prefix = $self->getPrefix($login, $hubtype, $keyfile);
		$self->setGitSsh($login, $hubtype, $keyfile);
	}
	my $command = "$prefix git pull git\@github.com:$owner/$repository.git 2> /dev/null ";
	$self->logDebug("command", $command);
	my ($output, $error) = $self->runCommand($command);

	#### GET TAGS
	$command = "$prefix git pull git\@github.com:$owner/$repository.git --tags 2> /dev/null ";
	$self->logDebug("command", $command);	
	($output, $error) = $self->runCommand($command);
