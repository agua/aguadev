...
method commitToRepo ($message) {
	...
	$command .= qq{ -m "$message"} if defined $message and $message;
...
method addLocalTag ($tag, $description) {
	...
	my $command = "git tag -a $tag";
	$command .= qq{ -m "$description"} if defined $description;
...
method pushTags ($login, $hubtype, $remote, $branch, $keyfile) {
	...
	my $gitssh = $self->setGitSsh($login, $hubtype);
