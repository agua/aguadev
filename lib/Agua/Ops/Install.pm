...
method setUpInstall {
	...
	$self->loadOpsModule($opsdir, $repository) if not $self->opsmodloaded();

	#### LOAD OPS INFO IF PRESENT
	$self->loadOpsInfo($opsdir, $repository) if not defined $self->opsinfo();
}
...
method runInstall {
	...
	if( $found == 0 ) {
		...
		$self->logDebug("FAILED to clone repo") and return if not $self->cloneRemoteRepo($owner, $repository, $hubtype, $login, $keyfile, $subdir);
	}
...
method loadOpsModule ($opsdir, $repository) {
	my $pmfile 	= 	"$opsdir/$repository.pm";
	my $location    = 	"$repository.pm";
	$self->logDebug("pmfile: $pmfile");
	
	if ( -f $pmfile ) {
		$self->logDebug("\nLoading modulefile: $repository.pm\n");
		unshift @INC, $opsdir;
		require $location;
		Moose::Util::apply_all_roles($self, $repository);
	}
	else {
		$self->logDebug("\nmodulefile not found: $repository.pm\n");
	}
	$self->opsmodloaded(1);
}

method loadOpsInfo ($opsdir, $repository) {
	$self->logDebug("opsdir", $opsdir);
	$self->logDebug("repository", $repository);
	
	return if not defined $opsdir or not $opsdir;
	my $opsfile 	= 	"$opsdir/$repository.ops";
