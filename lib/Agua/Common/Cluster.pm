...
sub clusterIsRunning {
#### CONFIRM CLUSTER IS RUNNING AND HAS NODES
	my $self		=	shift;
	my $cluster		=	shift;
	my $configfile	=	shift;
	my $starcluster	=	shift;
	$self->logDebug("Agua::Common::Cluster::clusterIsRunning(cluster, configfile, starcluster)");
	$self->logDebug("cluster", $cluster) if defined $cluster;
	$self->logDebug("configfile", $configfile) if defined $configfile;
	$self->logDebug("starcluster", $starcluster) if defined $starcluster;
	...
	my $running = $clusterinstance->isRunning();
	$running = 0 if not defined $running;

	my $hasnodes = $clusterinstance->hasNodes();
...
sub addCluster {
	my $self		=	shift;
 	$self->logDebug("Common::addCluster()");
    my $data 		=	$self->json();
...
sub newCluster {
	my $self		=	shift;
        ...
        my $json 		=	$self->json();
 	$self->logDebug("Common::newCluster()");	;
	my $username 	=	$json->{username};
	my $cluster 	=	$json->{cluster};
	...
	return if not defined $success or not $success;
...
sub getMasterId {
	...
	my $starcluster = $self->conf()->getKey("applications:$applications", "STARCLUSTER");
