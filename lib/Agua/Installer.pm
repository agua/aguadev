...
sub install {
    ...
    ####### CREATE JBROWSE DIRECTORIES 
    #$self->createJBrowseDirs();
...
sub installOpsRepo {
	...
	my $opsrepo 	= 	$conf->getKey("agua", "OPSREPO");
	my $owner		=	"agua";
	my $username	=	$self->get_username();
	my $basedir 	= 	$self->get_installdir();
	my $installdir 	= 	"$basedir/repos/public/$owner/$opsrepo";
	print "Agua::Installer::installOpsRepo    opsrepo: $opsrepo\n";
	print "Agua::Installer::installOpsRepo    owner: $owner\n";
	print "Agua::Installer::installOpsRepo    installdir: $installdir\n";
	...
	return $self->installApplication($owner, $username, $opsrepo, $installdir);
}
...
sub installApplication {
	...
	my $versions 	=	$self->getVersions($owner, $repository);
	my $version = shift @$versions;
	my $command = $self->opsCommand($owner, $repository, $installdir, $opsdir);
	print `$command`;
}
...
sub getVersions {
	...
	my $command = qq{$ops \\
$repository getRemoteTags $owner $repository
};
	print "Agua::Installer::getVersions    command: $command\n";
	my $result = `$command`;	
	print "Agua::Installer::getVersions    result: $result\n";
	...
	print "Agua::Installer::getVersions    BEFORE versions: @$versions\n";
	$versions = $self->sortVersions($versions);
	print "Agua::Installer::getVersions    AFTER versions: @$versions\n";
...
--logfile /tmp/agua-$repository.install.log \\
	...
	print "Agua::Installer::opsCommand    command: $command\n";
...
sub installBioApps {
	...
	my $installdir = $self->get_installdir();
	print "Agua::Installer::installBioApps    installdir: $installdir\n";

	require Conf::Agua;
	my $conf 	= Conf::Agua->new({inputfile=>"$Bin/../../conf/default.conf"});
	my $opsrepo = $conf->getKey("agua", "OPSREPO");
	my $package = $conf->getKey("agua", "APPSREPO");
	print "Agua::Installer::installBioApps    package: $package\n";
	my $owner	=	"agua";
	print "Agua::Installer::installBioApps    owner: $owner\n";

	my $opsdir 	=	"$installdir/repos/public/$owner/$opsrepo/$owner/$package";
	print "Agua::Installer::installBioApps    opsdir: $opsdir\n";

	return $self->installApplication($owner, $package, "$installdir/$package", $opsdir);	
}
