...
has 'starcluster'	=> ( is  => 'rw', 'isa' => 'Str|Undef', required	=>	0	);
...
method initialise () {
	...
	$self->starcluster($self->conf()->getKey("applications:aquarius-8", "STARCLUSTER"));
...
=cut
	$self->logDebug("StarCluster::writeConfigfile()");
...
#### 2. START BALANCER IF BALANCER NOT RUNNING 
	...
	$self->logError("starcluster not defined") and exit if not $starcluster;
	...
	##### DEBUG
	#$cluster_running = 0;
	#$self->logDebug("DEBUG cluster_running", $cluster_running);
	##### DEBUG
	##### DEBUG
	##### DEBUG
	...
	if ( not $cluster_running ) {
		...
		$self->terminateCluster($cluster, $configfile, $starcluster, undef, undef);
		
		#### TERMINATE BALANCER IF EXISTS
		$self->terminateBalancer($username, $cluster);
		
		#### START CLUSTER
		$started = $self->launchCluster($username, $cluster, $configfile, $starcluster, $outputdir);
	}
...
method launchCluster ($username, $cluster, $configfile, $starcluster, $outputdir) {
	$self->logDebug("username", $username);
	$self->logDebug("cluster", $cluster);
	$self->logDebug("configfile", $configfile);
...
#### DEBUG
	...
	my $command = "cd $configdir/plugins; $starcluster -c $configfile --logfile $logfile start $cluster";
...
method stop () {
	...
	return if not $self->terminateCluster($username, $cluster, $configfile, $starcluster, undef, undef);
...
method terminateCluster ($cluster, $configfile, $starcluster, $tries, $delay) {
	...
	### TERMINATE THE CLUSTER	
	my $command = "$starcluster -c $configfile terminate --confirm $cluster 2>&1";
	$self->logDebug("command", $command);
